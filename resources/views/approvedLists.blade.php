            ...
            <div class="col-md-12" id="mainBody">
                ...
                <center><table id="rooms">
                        ...
                        <tr>
                            ...
                            <th>Room Number</th>
                            <th>Book Id</th>
                            <th>Sender of request</th>
                            <th>Name of landlord approving</th>
                        </tr>
                        </thead>
                        <tbody>
                        @foreach($approvedRequests as $appr)
                            <tr>
                                <td>{{$appr->hostels->hname}}</td>
                                <td>{{$appr->room->rno}}</td>
                                <td>{{$appr->book_id}}</td>
                                <td>{{$appr->userClient->fname}} {{$appr->userClient->lname}}</td>
                                <td>{{$appr->user->fname}} {{$appr->user->lname}}</td>
                            </tr>
                        @endforeach
                        </tbody>
                    </table></center>
